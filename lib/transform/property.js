...
const mapPropValue = require('../utils/mapPropValue');
  ...
  for (let i = 0; i < transforms.length; i++ ) {
    ...
    if (!transform.matcher || transform.matcher(to_ret)) {
      ...
      if (transform.type === 'value') {
        const valueFields = options.valueTransformFields
        valueFields
          .filter((valueField) => Object.hasOwn(property, valueField))
          .forEach((valueField) => {
            if (!usesReference(property[valueField], options)) {
              // Only transform non-referenced values (from original)
              // and transitive transforms if the value has been resolved
              if (!usesReference(property.original[valueField], options) || transform.transitive) {
                to_ret[valueField] = transform.transformer(
                  mapPropValue(to_ret, [valueField], valueFields),
                  options
                )
              }
            }

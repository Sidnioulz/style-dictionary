...
`;
  ...
  if (to_ret.valueTransformFields) {
    // Harmonise type and typecheck
    if (_.isString(to_ret.valueTransformFields)) {
      to_ret.valueTransformFields = [to_ret.valueTransformFields]
    }
    if (!(_.isArray(to_ret.valueTransformFields))) {
      throw new Error(`
      Option "valueTransformFields" must be a string or a string array. Received: ${typeof to_ret.valueTransformFields}.`);
    }

    // Ensure fields declared for value transforms are not reserved keywords
    ['name', 'comment', 'themeable', 'attributes', 'original'].forEach((reservedWord) => {
      if (to_ret.valueTransformFields.includes(reservedWord)) {
        throw new Error(`
Field "${reservedWord}" found in option "valueTransformFields": this field name is reserved and cannot be transformed as a value.`)
      }
    })
  } else {
    // If omitted, init to default so we can rely on valueTransformFields being set later
    to_ret.valueTransformFields = ['value']
  }

## Transforming extra value fields

This example shows how to apply value transforms to multiple fields of your tokens instead of only the *value* field.

Traditionally, when token values depend on a context, it is recommended to create multiple token folders and to run Style Dictionary on each folder. This can include [multi-brand systems](../multi-brand-multi-platform), dark or high-contrast mode, or language-specific typography.

Alternatively, you can apply value transforms to multiple fields, and then call [custom formats](../custom-formats-with-templates/) multiple times to generate an output file for each value field. The benefit of this approach is that it lets you store all values for a token in the same place, which can ease the maintenance of large token systems.

Applying a value transform to multiple fields is different from applying [attribute transforms](../../../docs/transforms.md?id=transform-types). You cannot apply existing value transforms to an attribute, for instance.
